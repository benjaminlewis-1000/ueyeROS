 ...
 * \li  05.02.2010 - created
 * \li  03.05.2010 - disbaled size controls for cameras not supporting a continuous AOI
 *                   -> than they uses special image formats
 * \li  24.04.2013 - Changes on Image-Size settings
 */
...
void properties::UpdateWindowSizeCtrls ()
{
    ...
    m_pSliderWidthEx->setMinValue (m_minWidth);
    m_pSliderWidthEx->setMaxValue (m_maxWidth);
    ...
    spinBoxWidth->setMinValue (m_minWidth);
    spinBoxWidth->setMaxValue (m_maxWidth);
    spinBoxWidth->setSingleStep (m_incWidth);
    spinBoxWidth->setValue (m_nWidth);
    // height
    m_pSliderHeightEx->EnableSignaling(false);
    m_pSliderHeightEx->setMinValue (m_minHeight);
    m_pSliderHeightEx->setMaxValue (m_maxHeight);
    ...
    spinBoxHeight->setMinValue (m_minHeight);
    spinBoxHeight->setMaxValue (m_maxHeight);
    spinBoxHeight->setSingleStep (m_incHeight);
    spinBoxHeight->setValue (m_nHeight);
    // xpos
    m_pSliderPosXEx->EnableSignaling(false);
    m_pSliderPosXEx->setMinValue (m_minXPos);
    m_pSliderPosXEx->setMaxValue (m_maxXPos);
    ...
    spinBoxXPos->setMinValue (m_minXPos);
    spinBoxXPos->setMaxValue (m_maxXPos);
    spinBoxXPos->setSingleStep (m_incXPos);
    spinBoxXPos->setValue (m_nXPos);
    // ypos
    m_pSliderPosYEx->EnableSignaling(false);
    m_pSliderPosYEx->setMinValue (m_minYPos);
    m_pSliderPosYEx->setMaxValue (m_maxYPos);
    ...
    spinBoxYPos->setMinValue (m_minYPos);
    spinBoxYPos->setMaxValue (m_maxYPos);
...
void properties::OnRadioHFormat ()
{
    ...
    // check image size
    int width = (int)((double)m_nWidth * m_hFactor);
    qDebug("new HFormat - old width: %d, hFactor: %d , min width: %d", width, hFactor, m_minWidth);
    if ((width / hFactor) < m_minWidth)
    {
        qDebug("warning - new width %d", (m_nWidth/hFactor));
        UpdateSizeFormatCtrls ();
        UpdateWindowSizeCtrls ();

        QMessageBox::information(this, tr("uEye camera demo"), tr("The binning / subsampling mode can not be set. "
                        "The size is less than the minimal image size"), 0);
        return;
    }
...
void properties::OnRadioVFormat ()
{
    ...
    // check image size
    int height = (int)((double)m_nHeight * m_vFactor);
    qDebug("new VFormat - old Height: %d, vFactor: %d , min Height: %d", height, vFactor, m_minHeight);
    if ((height / vFactor) < m_minHeight)
    {
        qDebug("warning - new height %d", (m_nHeight/vFactor));
        UpdateSizeFormatCtrls ();
        UpdateWindowSizeCtrls ();
