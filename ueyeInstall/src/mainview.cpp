 ...
 * \version 1.13.1
 *
 * \brief   MainWindow class implementation of the Qt uEye demo
 *
 * \par Revision history
 * \li  26.06.2009 - created
 * \li  18.08.2009 - (1.0.1) statusbar shows now "camera open" message
 * \li  19.08.2009 - (1.0.2) statusbar shows now a progressbar, when opening
 *                               a camera.
 * \li  19.08.2009 - (1.0.3) with menu->uEye->force cameralist the cameralist
 *                               is always shown, when opening a camera.
 * \li  19.08.2009 - first camera has to be selected, when no list is shown
 * \li  19.08.2009 - (1.0.5) added Demo Version in titlebar
 * \li  30.09.2009 - (1.1.0) added support for new api 3.40 event IS_SET_EVENT_STATUS_CHANGED
 * \li  02.10.2009 - (1.1.1) changed textlabel behavior (API Version) is not be overwritten.
 * \li  15.10.2009 - (1.1.2) fix mantis 0001640
 * \li  19.10.2009 - (1.1.3) the "failed" counter is now updated with the correct API value from
 *                               functions is_GetCaptureErrorInfo. (mantis 0001630)
 * \li  21.10.2009 - (1.1.4) enable continuous trigger. This fixes ticket 1688.
 * \li  21.10.2009 - (1.1.5) add an eventthread for event IS_SET_EVENT_TRANSFER_FAILED to update the
 *                               failed counter when no frames are reported.
 * \li  26.10.2009 - (1.2.0) add commandline option -s\<deviceid\> to start a camera at startup
 * \li  29.10.2009 - (1.3.0) add modeless imageinfo dlg
 * \li  03.11.2009 - (1.4.0) add possibility to start camera live or in stop mode
 * \li  18.11.2009 - (1.4.1) use new 'IS_INVALID_HIDS' instead of 'NULL' for HIDS compare
 * \li  27.11.2009 - (1.5.0) add automatic firmware support
 * \li  01.12.2009 - (1.6.0) new function "ChangeImageSize" to allocate new sized buffers, cause
 *                       of size changes in property page. So display and save show only AOI data now!
 * \li  14.12.2009 - (1.6.1) - onLoad/SaveParameter handles now "ini" as default suffix.
 *                               - onSaveImage handles now bmp, jpeg, png
 * \li  15.12.2009 - (1.6.2) images will be resized now, when new parameter file is loaded.
 * \li  16.12.2009 - (1.6.3) reset camera to defaults after opening
 *                 - (1.6.4) fix incorrect type of NULL camera handle in assignments.
 * \li  23.12.2009 - (1.7.0) use a QScrollArea and a QLabel as paint view.
 * \li  23.12.2009 - (1.7.1) disable buttons to open a new camera, when opening is in progress.
 * \li  12.01.2010 - (1.8.0) add display rate limit and display counter in statusbar
 * \li  03.02.2010 - (1.8.1) add error checking for loading camera parameter files
 * \li  03.02.2010 - (1.9.0) - Load and save Parameter file is now also possible when camera is capturing
 *                               - add functionality to reset the camera to its defaults
 * \li  04.02.2010 - (1.9.1) SetupCapture don't start/stop the live capture anymore to avoid problems with
 *                               the live button status
 * \li  05.02.2010 - (1.10.0) add load/save camera parameter using the internal memory
 * \li  05.02.2010 - (1.10.1) save image is now possible also when camera is capturing
 * \li  15.02.2010 - (1.10.2) change mainwindow background to show ids ueye logo
 * \li  15.02.2010 - (1.10.3) set display true, when camera is closed and opened
 * \li  30.04.2010 - (1.11.0) add UI1008XS image format support
 * \li  03.05.2010 - (1.11.1) check the sensorinfo colormode for modi greater than bayer to find colorformats,
 *                            cause the XS uses IS_COLORMODE_CBYCRY
 * \li  06.05.2010 - (1.11.2) bugfix using function is_GetActSeqBuf. It is strongly recommended to initialize the function parameter
 *                            with valid values. The unused parameters should be "0", cause they could be processed internally.
 * \li  22.09.2010 - (1.11.3) change QuEyeSdiDemo to uEyeDemo
 * \li  31.01.2011 - (1.12.0) - changed: reject opening an unavailable device.
 * \li                        - fixed some issues on statusbar info.
 * \li  15.04.2011 - (1.12.1) - save && read window state
 * \li  17.06.2011 - (1.13.0) - removed openmp
 * \li  05.08.2011 - (1.13.1) - do not set camera parameters to default after opening if there is an
 *                              'initial parameter set' installed.
 * \li  22.03.2012 - (1.13.2) - add openmp
 * \li  04.05.2012 - (1.13.3) - some fixes
 * \li  16.05.2012 - (1.14.0) - Changes to use new functions
 * \li  08.11.2012 - (1.14.1) - Update to new color mode defines
 * \li  11.12.2012 - (1.14.2) - fixes between USB3 Camera and StopLiveVideo
 * \li  03.06.2013 - (1.14.3) - changes to get Default AOI
 *                              add function to print Titles
 * \li  05.02.2014 - (1.14.4) - Implemented faster eventthread teardown. Use EVENTTHREAD_WAIT_TIMEOUT for qthread::wait().
 * \li  13.02.2014 - (1.14.5) - Disable "Load Parameterset" action if the parameterset was deleted.
 * \li  15.05.2014 - (1.14.6) - Call Default-Function from Property-Page if ResetToDefault is pushed
 * \li  19.05.2014 - (1.14.7) - Patch ColorMode RGB5 to RGB565
 */
    ...
    QMainWindow (parent)
{
    ...
    QString strWindowTitle = QString ("uEye Demo %1").arg(DEMO_VERSION_STR);
...
void Mainview::about ()
{
    QMessageBox::about (this, "About uEye Demo",
                        QString("uEye Demo application %1\n"
                        "IDS Imaging Development Systems GmbH").arg(DEMO_VERSION_STR));
}
...
void Mainview::onCloseCamera()
{
    ...
    m_pEvTransferFailed->stop ();
    /* stop event threads in the property dialog */
    m_pPropertiesDlg->SignalStopEventThreads();
...
{
    QString strWindowTitle = QString ("uEye Demo %1 - %2 (%3x%4) - ID: %5 - SerNo: %6")
        .arg(DEMO_VERSION_STR).arg(m_CamListInfo.Model).arg(m_BufferProps.width).arg(m_BufferProps.height)
...
int Mainview::SetupCapture()
{
    ...
    if (nRet == IS_SUCCESS)
    {
        ...
        if(colormode == IS_CM_BGR5_PACKED)
        {
            ...
            QMessageBox::information(this, "Info!", QString("Colorformat 'IS_CM_BGR5_PACKED' actually not supported from Demo, patched to 'IS_CM_BGR565_PACKED'\n"));
...
void Mainview::onLoadParameter(QAction* action)
{
    ...
    if (!fileName.isEmpty() || action == actionLoadParameterSet)
    {
        ...
        if (ret == IS_SUCCESS)
        {
            ...
            if(IS_CONV_MODE_JPEG & nCurrentConvertMode)
            {
                m_pPropertiesDlg->OnXsJPEGMode(true);
            }
            else
            {
                m_pPropertiesDlg->OnXsJPEGMode(false);
            }
        }
