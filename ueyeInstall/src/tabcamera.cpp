 ...
 * \li  03.02.2011 - fixed: turn off auto framerate if auto exposure is turned off.
 * \li  15.04.2011 - automatic control update
 * \li  04.05.2012 - changes on Pixelclock settings
 * \li  08.05.2012 - fixes to set only supported Pixelclocks
...
void properties::initTabCamera()
{
    ...
    int nRet = is_PixelClock(m_hCamera, IS_PIXELCLOCK_CMD_GET_NUMBER,
...
void properties::UpdateFramerateCtrls ()
{
    ...
    checkAutoFps->setEnabled(m_bAutoExposure && !m_bAutoGain);
    // spin box
...
void properties::UpdateExposureCtrls ()
{
    ...
    is_SetAutoParameter (m_hCamera, IS_GET_ENABLE_AUTO_SHUTTER, &dblEnable, NULL);
    m_bAutoExposure = (dblEnable != 0.0);
...
void properties::checkAutoExp_clicked ()
{
    ...
    if (is_SetAutoParameter(m_hCamera, IS_SET_ENABLE_AUTO_SHUTTER, &dEnable, NULL) != IS_SUCCESS)
    {
        QMessageBox::information(0, "Error !!!", "SetAutoShutter failed", 0);
    }
