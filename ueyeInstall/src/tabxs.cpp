 ...
 *
 * \brief   properties tab widget for special ueye xs camera features
 *
 * \par Revision history
 * \li  18.05.2010 - created
 * \li  28.05.2013 - Update to support new Functions
 * \li  03.06.2013 - some fixes
 *                   add new XS-Functions
 * \li  28.01.2014 - Update color formats widget when switching jpeg mode
 *                 - Disable jpeg check box when a not supported jpeg color mode is set
 */
...
void properties::initXs_AutoParamControls()
{
    ...
    if (is_GetAutoInfo(m_hCamera, &autoInfo) == IS_SUCCESS)
    {
        ...
        {
            // will be removed on 4.40
            dEnable = 1;
            is_SetAutoParameter(m_hCamera, IS_SET_ENABLE_AUTO_SENSOR_GAIN_SHUTTER, &dEnable, NULL);
